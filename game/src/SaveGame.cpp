...
std::shared_ptr<NoLifeNoCry::Game::SaveGame> NoLifeNoCry::Game::SaveGame::Load(const std::filesystem::path& saveGameFilePath)
{
	...
	try
	{
			...
			std::filesystem::is_regular_file(saveGameFilePath))
		{
			...
			std::shared_ptr<NoLifeNoCry::Game::SaveGameData> save_game_data(NoLifeNoCry::Engine::XMLSerialiser<NoLifeNoCry::Game::SaveGameData>().DeserialiseObject(&xml_document));
			if (save_game_data)
			{
...
bool NoLifeNoCry::Game::SaveGame::Save(const std::filesystem::path& path)
{
	...
	try
	{
		...
		{
			std::shared_ptr<rapidxml::xml_document<>> xml_document(NoLifeNoCry::Engine::XMLSerialiser<SaveGameData>().SerialiseObject(saveGameData));
			if (xml_document)
			{
